...
def _send_notification(data):
    ...
    if not bearer_token:
        logger.warning("SLACK_BOT_TOKEN not set, will not send any Slack notifications.")
        return
...
class SlackNotification(NotificationPlugin):
    ...
    @staticmethod
    def send(notification_type, message, excluded_targets, options, endpoint=None, **kwargs):
        ...
        if notification_type == "rotation":
            ...
            return (
                SlackNotification
                .send_rotation_notification(channel, message, endpoint))
